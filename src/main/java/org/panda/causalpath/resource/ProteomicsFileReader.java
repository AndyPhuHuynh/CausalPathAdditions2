...
public class ProteomicsFileReader
{
	...
	{
		return readAnnotation(filename, idCol, symbolCol, siteCol, modCol, effectCol, null);
	}
	 ...
	 */
	public static List<ProteomicsFileRow> readAnnotation(String filename, String idCol, String symbolCol,
		String siteCol, String modCol, String effectCol, CPLogger logger) throws FileNotFoundException
	{
		...
		if (colInd == -1 && logger != null) logger.dataError.error("ID column '" + idCol + "' not found in data file '" + filename + "'.");
		if (symbolInd == -1 && logger != null) logger.dataError.error("Symbol column '" + symbolCol + "' not found in data file '" + filename + "'.");
		if (siteInd == -1 && logger != null) logger.dataError.error("Site column '" + siteCol + "' not found in data file '" + filename + "'.");
		if (modInd == - 1 && modCol != null && logger != null)
			logger.dataError.error("Provided feature column '" + modCol + "' but column was not found in the data file '" + filename + "'.");
		if (effectInd == - 1 && effectCol != null && logger != null)
			logger.dataError.error("Provided effect column '" + effectCol + "' but column was not found in the data file '" + filename + "'.");
		...
		while (sc.hasNextLine())
		{
			...
			if (modInd >= 0 && row.length > modInd)
			{
				...
				{
					logger.dataWarning.warn("Line " + line + ": Feature '" + row[modInd] + "' is not recognized. Defaulting to no feature." +
			...
			if (effectInd >= 0 && row.length > effectInd)
			{
				...
				{
					logger.dataWarning.warn("Line " + line + ": Effect '" + row[effectInd] + "' is not recognized. Defaulting to no effect." +
